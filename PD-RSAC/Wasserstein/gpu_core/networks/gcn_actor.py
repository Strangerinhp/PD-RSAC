...
class GCNActor(nn.Module):
    ...
    def __init__(
        ...
        # ===== Action Type Head =====
        # Discrete: IDLE, SERVE, CHARGE, REPOSITION
        ...
        # Initialize action head with bias toward SERVE
        with torch.no_grad():
            self.action_type_head.bias[1] = 1.5  # Bias toward SERVE
    ...
    def forward(
        ...
        action_probs = torch.nan_to_num(action_probs, nan=0.25)  # Uniform if NaN
        
        # Handle all-masked case: set default IDLE action
        all_masked = action_probs.isnan().any(dim=-1) | (action_probs.sum(dim=-1) == 0)  # [batch, num_vehicles]
        if all_masked.any():
            # Create one-hot for IDLE action
            default_probs = torch.zeros_like(action_probs)  # [batch, num_vehicles, action_dim]
            default_probs[..., 0] = 1.0  # IDLE action
            # Replace masked vehicles with default
        ...
        # Entropy: -sum(p * log(p))
        action_entropy = -(action_probs * action_log_softmax).sum(dim=-1)  # [batch, num_vehicles]
        
        # ===== Reposition Head (MEMORY-EFFICIENT) =====
        # Only compute reposition for vehicles that chose REPOSITION (action_type == 3)
        ...
        reposition_mask_action = (action_type == 3)  # [batch, num_vehicles]
        ...
        if reposition_mask_action.any():
            ...
            repos_probs = F.softmax(repos_logits / temperature, dim=-1)
            repos_probs = torch.nan_to_num(repos_probs, nan=1.0 / self.num_hexes)

            if deterministic:
                repos_target = repos_probs.argmax(dim=-1)
            else:
                repos_target = Categorical(probs=repos_probs).sample()
        ...
        # Only compute for vehicles that chose SERVE (action_type == 1)
        ...
        if compute_trips:
            serve_mask_action = (action_type == 1)  # [batch, num_vehicles]
            ...
            if serve_mask_action.any():
                ...
                if trip_mask is not None:
                    if trip_mask.dim() == 1:
                        ...
                        serve_trip_logits = serve_trip_logits.masked_fill(~trip_mask[batch_idx_s], float('-inf'))
    
                serve_trip_probs = F.softmax(serve_trip_logits / temperature, dim=-1)
                serve_trip_probs = torch.nan_to_num(serve_trip_probs, nan=1.0 / self.max_trips)
    
                if deterministic:
                    serve_trip_target = serve_trip_probs.argmax(dim=-1)
                else:
                    serve_trip_target = Categorical(probs=serve_trip_probs).sample()
...
class GCNActorWrapper(nn.Module):
    ...
    def __init__(
        ...
        # Initialize with SERVE bias
        with torch.no_grad():
            self.action_type_head.bias[1] = 2.0
    ...
    def forward(
        ...
        reposition_probs = F.softmax(reposition_logits, dim=-1)
        reposition_dist = Categorical(probs=reposition_probs)
        
        if deterministic:
            reposition_target = reposition_probs.argmax(dim=-1)
        else:
            reposition_target = reposition_dist.sample()
        
        reposition_log_prob = reposition_dist.log_prob(reposition_target)
