...
class HeuristicMatcher:
    ...
    def select_actions_simple(
        ...
        # Phase 1: Critical vehicles MUST charge
        critical_mask = vehicle_socs < self.critical_soc_threshold
        critical_indices = available_vehicles[critical_mask]
        charge_vehicles = critical_indices.clone()
        
        # Phase 2: Optimized greedy serving for remaining vehicles
        remaining_mask = ~critical_mask
    ...
    def select_actions(
        ...
        # Phase 1: Critical vehicles MUST charge (SOC < threshold)
        critical_mask = vehicle_socs < self.critical_soc_threshold
        critical_indices = available_indices[critical_mask]
        ...
        remaining_mask = ~critical_mask if len(critical_indices) > 0 else torch.ones(len(available_indices), dtype=torch.bool, device=device)
