...
from gpu_core.training.curriculum import CurriculumScheduler, apply_curriculum_to_environment
...
from gpu_core.simulator.orchestrator import SimulatorOrchestrator
from gpu_core.data.episode_data import EpisodeDataManager
...
def parse_args():
    ...
    parser.add_argument('--smart-assignment', action='store_true', default=True,
                        help='Enable smart auction-based assignment (~95%% MILP optimality)')
    parser.add_argument('--no-smart-assignment', dest='smart_assignment', action='store_false',
                        help='Disable smart assignment (use random sampling)')
...
def create_agent(config: Config, device: torch.device, env=None) -> SACAgent:
    ...
    # Set min_alpha from config
    if hasattr(agent, 'min_alpha'):
        agent.min_alpha = entropy_config.min_alpha
...
def train_single_gpu(args):
    ...
    if use_enhanced:
        enhanced_config = EnhancedTrainingConfig(
            ...
            use_smart_assignment=args.smart_assignment,
        ...
        print(f"[Enhanced] Semi-MDP: {args.semi_mdp}, WDRO: {args.wdro}, Smart Assignment: {args.smart_assignment}")
    ...
    if args.semi_mdp:
        ...
        if args.smart_assignment:
            print(f"[Smart Assignment] Enabled (~95% MILP optimality)")
    ...
    try:
        ...
        csv_writer.writerow([
            ...
            'Alpha', 'Idle_Pct', 'Serve_Act_Pct', 'Charge_Pct', 'Repos_Pct'
        ])
    ...
    curriculum = CurriculumScheduler.from_config(config, config.training.total_episodes)
    if curriculum.enabled:
        print(f"[Curriculum] Enabled: pickup_distance {curriculum.pickup_start:.1f}km -> {curriculum.pickup_end:.1f}km")
        print(f"[Curriculum] Schedule: {curriculum.schedule}, ends at episode {curriculum.curriculum_end_episode}")
    ...
    for episode in range(start_episode, config.training.total_episodes):
        # Apply curriculum learning - update max pickup distance
        if curriculum.enabled:
            pickup_dist = apply_curriculum_to_environment(
                env, curriculum, episode,
                verbose=(episode % args.log_interval == 0)
            )
        ...
        if (episode + 1) % args.log_interval == 0:
            ...
            if action_counts:
                ...
                action_str = f"I:{action_pct['idle']:4.1f}% S:{action_pct['serve']:4.1f}% C:{action_pct['charge']:4.1f}% R:{action_pct['reposition']:4.1f}%"
            else:
                action_str = ""
            ...
            if writer:
                ...
                # Log curriculum
                if curriculum.enabled:
                    curr_state = curriculum.get_state(episode)
                    writer.add_scalar('Curriculum/pickup_distance', curr_state.pickup_distance, episode + 1)
                    writer.add_scalar('Curriculum/progress', curr_state.progress, episode + 1)
            ...
            with torch.no_grad():
                ...
                if hasattr(env, 'trip_state'):
                    ...
                    if unassigned_mask.any():
                        ...
                        trip_mask = torch.zeros(max_trips, dtype=torch.bool, device=device)
    ...
    try:
        ...
        if args.semi_mdp:
            ...
            collector = EnhancedEpisodeCollector(
                ...
                use_smart_assignment=args.smart_assignment, track_durations=True
            )
        else:
        ...
        # === Curriculum ===
        curriculum = CurriculumScheduler.from_config(config, config.training.total_episodes)
        if rank == 0 and curriculum.enabled:
            print(f"[Curriculum] {curriculum.pickup_start:.1f}km -> {curriculum.pickup_end:.1f}km, ends at ep {curriculum.curriculum_end_episode}")
        ...
        for episode in range(start_episode, num_episodes):
            # Apply curriculum
            if curriculum.enabled:
                apply_curriculum_to_environment(
                    env, curriculum, episode,
                    verbose=(rank == 0 and episode % args.log_interval == 0)
                )
            ...
            episode_stats = collector.collect_episode(
                ...
                seed=args.seed + episode, temperature=1.0
            )
            total_steps += episode_stats.steps
