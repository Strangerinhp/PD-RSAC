...
class GPUEnvironmentV2:
    ...
    def step(
        ...
        # Create action masks
        idle_mask = (action_type == 0) & available_mask
        serve_mask = (action_type == 1) & available_mask
        charge_mask = (action_type == 2) & available_mask
        reposition_mask = (action_type == 3) & available_mask
        ...
        # 2. CHARGE actions
        charge_cost, num_charge_failed, charge_veh_idx, charge_station_idx = \
        ...
        if reposition_target is not None:
            ...
            # Reposition penalty (avoid .item() - keep as tensor)
        ...
        self._action_processor.update_ongoing_actions(self.current_step)
        ...
        # IDLE penalty (including failed actions) - keep as tensors
        idle_penalty_rate = self._reward_computer.idle_penalty
        num_idle = idle_mask.sum()
        num_effectively_idle = num_idle + num_serve_failed + num_charge_failed
        idle_penalty = num_effectively_idle * idle_penalty_rate
        reward -= idle_penalty
        
        # High SOC charge penalty
        ...
        if self.current_step % 20 == 0 and self.current_step > 0:
            print(f"  [Step {self.current_step}] Revenue: {trip_revenue.item():.1f}, "
                  ...
                  f"Charge: {charge_cost.item():.1f}, "
                  f"ReposBonus: {reposition_bonus.item():.2f}, "
                  f"ServeFail: {num_serve_failed}, ChargeFail: {num_charge_failed}, "
                  f"Wait: {wait_penalty.item():.1f}, "
                  f"Idle: {idle_penalty:.1f}, "
        ...
        self._update_info(trips_served, trip_revenue.item(), serve_driving_cost.item(), charge_cost.item())
    ...
    def get_action_distribution(self, action_type: torch.Tensor) -> Dict[str, float]:
        ...
        if total == 0:
            return {"idle": 0, "serve": 0, "charge": 0, "reposition": 0}
        
        return {
            "idle": (action_type == 0).sum().item() / total * 100,
            "serve": (action_type == 1).sum().item() / total * 100,
            "charge": (action_type == 2).sum().item() / total * 100,
            "reposition": (action_type == 3).sum().item() / total * 100,
        }
    ...
    def get_available_actions(self) -> torch.Tensor:
        ...
        # Low SOC vehicles can't SERVE (1) or REPOSITION (3)
        low_soc_mask = self.fleet_state.socs < self.config.vehicle.soc_low_threshold
        mask[low_soc_mask, 1] = False
        mask[low_soc_mask, 3] = False
        
        # High SOC vehicles shouldn't CHARGE (2)
        high_soc_mask = self.fleet_state.socs > 90
        mask[high_soc_mask, 2] = False
