...
class TensorTripState:
    ...
    def add_trips(
        ...
        n_new = len(trip_ids)
        available_slots = self.max_trips - self.num_trips
        n_add = min(n_new, available_slots)
        
        if n_add == 0:
            return 0
        
        start = self.num_trips
        end = start + n_add
        
        self.trip_ids[start:end] = trip_ids[:n_add]
        self.pickup_hex[start:end] = pickup_hexes[:n_add]
        self.dropoff_hex[start:end] = dropoff_hexes[:n_add]
        self.fare[start:end] = fares[:n_add]
        self.distance_km[start:end] = distances[:n_add]
        self.wait_steps[start:end] = 0
        self.assigned_vehicle[start:end] = -1
        self.assigned[start:end] = False
    ...
    def drop_expired(self, max_wait: int) -> int:
        ...
        # Compact the buffer to reuse slots
        self._compact()
