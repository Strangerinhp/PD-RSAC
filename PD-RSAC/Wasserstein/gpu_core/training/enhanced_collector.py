...
class TransitionWithDuration:
    ...
    action_type: int  # 0=IDLE, 1=SERVE, 2=CHARGE, 3=REPOSITION
...
class EnhancedReplayBuffer:
    ...
    def __init__(
        ...
        # Duration storage as tensor for fast indexing
        # Pre-allocate to buffer capacity
        capacity = base_buffer.capacity
        self.durations = torch.zeros((capacity, num_vehicles), device=device)
        self.action_types_storage = torch.zeros((capacity, num_vehicles), dtype=torch.long, device=device)
    ...
    def push(
        ...
        # Push to base buffer
        self.base_buffer.push(state, action, reward, next_state, done)
        
        # Store duration info in preallocated tensor
        idx = self.buffer_size % self.durations.shape[0]
        self.durations[idx] = duration
        self.action_types_storage[idx] = action_types
    ...
    def _update_duration_stats(self, duration: torch.Tensor, action_types: torch.Tensor):
        ...
        for action_idx, name in [(1, 'serve'), (2, 'charge'), (3, 'reposition')]:
            mask = action_types == action_idx
            if mask.any():
                self.duration_stats[name]['sum'] += duration[mask].sum().item()
    ...
    def sample(self, batch_size: int) -> Tuple[Dict, torch.Tensor, torch.Tensor]:
        ...
        # Get duration info for these indices - direct tensor indexing (fast!)
        durations = self.durations[indices]
        scenarios = torch.tensor([self.scenarios[i.item()] for i in indices], device=self.device)
    ...
    def sample_by_scenario(self, batch_size: int, scenario: int) -> Tuple[Dict, torch.Tensor]:
        ...
        # OPTIMIZED: Use tensor operations instead of list comprehension
        scenarios_tensor = torch.tensor(self.scenarios[:self.buffer_size], device=self.device)
        ...
        # Sample from scenario indices
        sample_positions = torch.randint(0, len(scenario_indices), (batch_size,), device=self.device)
        actual_indices = scenario_indices[sample_positions]
        
        base_batch = self.base_buffer.sample(batch_size)
        durations = self.durations[actual_indices]  # Direct tensor indexing
...
class EnhancedEpisodeCollector(EpisodeCollector):
    ...
    DURATION_PARAMS = {
        'idle_steps': 1.0,           # IDLE duration in steps
        'pickup_speed_hex_per_step': 2.0,  # Hex distance per step to pickup
        'reposition_speed_hex_per_step': 2.0,
        'charge_rate_soc_per_step': 5.0,   # SOC% gained per step
    ...
    def __init__(
        ...
        super().__init__(env, replay_buffer, device, use_smart_assignment)
        ...
        while not done:
            ...
            with torch.no_grad():
                if self.use_smart_assignment and hasattr(self, '_assigner'):
                    # Smart assignment not supported after EnhancedActor removal
            ...
            # Push to enhanced buffer with duration
    ...
    def _estimate_durations(
        ...
        # IDLE (action 0): fixed short duration
        idle_mask = action_type == 0
        durations[idle_mask] = params['idle_steps']
        
        # SERVE (action 1): trip duration + pickup time
        serve_mask = action_type == 1
        if serve_mask.any():
            # Estimate based on position change
        ...
        # CHARGE (action 2): based on SOC gained
        ...
        # REPOSITION (action 3): based on distance
        ...
        if reposition_mask.any():
            if self._distance_matrix is not None:
                ...
                repo_duration = distances / params['reposition_speed_hex_per_step']
                repo_duration = repo_duration.clamp(min=1.0)
            else:
                repo_duration = torch.ones(reposition_mask.sum(), device=self.device) * 2.0
            
            durations[reposition_mask] = repo_duration
    ...
    def _update_duration_tracking(
        ...
        action_names = {1: 'serve', 2: 'charge', 3: 'reposition'}
...
def create_enhanced_collector(
    ...
    Args:
        ...
        use_smart_assignment: Whether to use auction-based assignment
    ...
    return EnhancedEpisodeCollector(
        ...
        device=device,
        use_smart_assignment=use_smart_assignment,
