...
class GCNLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            adj: Adjacency matrix [num_nodes, num_nodes] (shared across batch)
        ...
        if support.dim() == 3:
            # Batched: support is [batch, num_nodes, features]
            # adj is [num_nodes, num_nodes]
            # Use einsum: adj[i,j] * support[batch,j,feat] -> output[batch,i,feat]
            output = torch.einsum('ij,bjf->bif', adj, support)
        else:
            # Non-batched: support is [num_nodes, features]
