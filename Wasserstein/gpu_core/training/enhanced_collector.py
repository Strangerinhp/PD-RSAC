...
class EnhancedReplayBuffer:
    ...
    def push(
        ...
        # Push to base buffer (including assignment info for auxiliary loss)
...
class EnhancedEpisodeCollector(EpisodeCollector):
    ...
    def _estimate_durations(
        ...
        # REPOSITION (action 2): based on distance
        reposition_mask = action_type == 2
        if reposition_mask.any():
            if self._distance_matrix is not None:
                # Use reposition_targets instead of new_positions for more accurate intent
                distances = self._distance_matrix[
