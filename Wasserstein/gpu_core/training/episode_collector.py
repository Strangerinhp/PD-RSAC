...
class EpisodeCollector:
    ...
    def __init__(
        ...
        if use_milp and MILPAssignment is not None:
            ...
            print(f"[Assignment] MILP assigner active (V={env.num_vehicles}, S={_num_sta}, delta_t={_delta_t:.4f}h, mu=10.0, max_pkup={_max_pkup:.1f}km)")
    ...
    def _select_batched_actions(
        ...
        for i in range(num_envs):
            if hasattr(agent, 'select_action'):
                ...
                actions[i, :, 0] = torch.randint(0, 3, (num_vehicles,), device=self.device).float()  # 3 actions: SERVE/CHARGE/REPOS
